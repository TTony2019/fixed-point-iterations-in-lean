...
theorem closed_is_weakly_closed [CompleteSpace H] (s : Set H)
  ...
  simp [IsWeaklyClosed]; refine {isOpen_compl := ?_}; refine isOpen_iff_forall_mem_open.mpr ?_
  intro x xsc; obtain ⟨f, u, fxu, fbu⟩ := geometric_hahn_banach_point_closed hs hw xsc
  let U := f⁻¹' (Set.Iio u)
  let yf := (InnerProductSpace.toDual ℝ H).symm f
  have (x : H) : ⟪yf , x⟫ = f x := InnerProductSpace.toDual_symm_apply
  let f1 := WeakSpace.map f; let f2 := (toWeakSpace ℝ ℝ).symm; let f21 := f2 ∘ f1
  have feq (x : H): f21 x = f x := rfl
  let U' := f21⁻¹' (Set.Iio u); use U'
  have U'Open : IsOpen U' := by
    refine Continuous.isOpen_preimage ?_ (Set.Iio u) isOpen_Iio; simp [f21]
    refine Continuous.comp ?_ (ContinuousLinearMap.continuous f1)
    simp [f2, continuous_real_weakspace]
  have hU'insc : U' ⊆ sᶜ := by
    intro g hg; simp; simp [U', feq g] at hg; by_contra! hgs; linarith [fbu g hgs]
  have hxinU' : x ∈ U' := Set.mem_preimage.mpr (by simp [feq x]; exact fxu)
  exact ⟨hU'insc, U'Open, hxinU'⟩
...
theorem weakly_closed_seq_closed (s : Set H) (hs : IsWeaklyClosed s) :
  ...
  simp [IsWeaklyClosed] at hs; simp [IsWeaklySeqClosed]; exact IsClosed.isSeqClosed hs
...
theorem browder_demiclosed_principle [CompleteSpace H]
  ...
  intro u h_D_nonempty h_D_weakly_seq_closed
    x hx_in_D x_lim hx_lim_in_D h_weak_conv h_diff_tendsto
  ...
  have h1 : Tendsto (fun n => ‖x n - T (x n) - u‖) atTop (𝓝 0) := by
    ...
    simp [dist_eq_norm] at ⊢ hN; exact hN

  have h2 : Tendsto (fun n => x n - T (x n) - u) atTop (𝓝 0) := by
    rw [Metric.tendsto_atTop]; intro ε ε_pos
    rw [Metric.tendsto_atTop] at h1; obtain ⟨N, hN⟩ := h1 ε ε_pos
    use N; intro n hn; specialize hN n hn
    simp [dist_eq_norm] at ⊢ hN; exact hN
  ...
  have h3 : WeakConverge (fun n => x n - x_lim) 0 := by
    ...
    have h_diff : Tendsto (fun n => ⟪x n, y⟫ - ⟪x_lim, y⟫) atTop (𝓝 (⟪x_lim, y⟫ - ⟪x_lim, y⟫)) :=
      Tendsto.sub h4 h5
    convert h_diff using 1; ext n; simp; rw [inner_sub_left]; ring_nf
  ...
  have h4 : WeakConverge (fun n => x n - T (x n)) u := by
    ...
    · have h2' : Tendsto (fun n => (x n - T (x n)) - u) atTop (𝓝 0) := by convert h2 using 1
      have h_inner : Tendsto (fun n => ⟪(x n - T (x n)) - u, y⟫) atTop (𝓝 0) := by
        rw [Metric.tendsto_atTop]; intro ε ε_pos; rw [Metric.tendsto_atTop] at h2'
        obtain ⟨N, hN⟩ := h2' (ε / ‖y‖) (by positivity); use N; intro n hn
        specialize hN n hn; simp [dist_eq_norm] at hN ⊢
        by_cases hy : y = 0
        · simp [hy]; linarith
        · calc
  ...
  have h5 : WeakConverge (fun n => T (x n) - x n + (x n - x_lim)
    ...
    have h_combined : Tendsto (fun n =>
      ...
      apply Tendsto.add (Tendsto.add h4_inner h3_inner) h_const
  ...
  have h8 : Tendsto (fun n ↦ ‖x n - T (x n) - u‖ ^ 2 +
    (2 * inner ℝ (T (x n) - T x_lim) (x n - T (x n) - u) -
      2 * inner ℝ (x n - x_lim) (x_lim - T x_lim - u))) atTop (𝓝 (0 + (0 - 0))) := by
        apply Tendsto.add h1' (Tendsto.sub ?_ h6)
  ...
  have h9 : ∀ ε > 0, ‖x_lim - T x_lim - u‖ ^ 2 < ε := by
    intro ε ε_pos; rw [Metric.tendsto_atTop] at h8'; obtain ⟨N, hN⟩ := h8' (ε) ε_pos
    specialize hN N (le_refl N); simp [dist_eq_norm] at hN; specialize h_norm_bound N
    calc
  ...
  have h_final : ‖x_lim - T x_lim - u‖ ^ 2 ≤ 0 := by
    apply le_of_forall_pos_le_add; intro ε ε_pos; specialize h9 ε ε_pos; linarith
  have h_nonneg : 0 ≤ ‖x_lim - T x_lim - u‖ ^ 2 := by apply pow_two_nonneg
  have : ‖x_lim - T x_lim - u‖ ^ 2 = 0 := by apply le_antisymm h_final h_nonneg
  have : ‖x_lim - T x_lim - u‖ = 0 := pow_eq_zero this
  have : x_lim - T x_lim - u = 0 := norm_eq_zero.mp this

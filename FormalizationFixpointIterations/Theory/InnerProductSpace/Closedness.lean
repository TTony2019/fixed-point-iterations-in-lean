...
theorem weakly_closed_seq_closed (s : Set H) (hs : IsWeaklyClosed s) :
  ...
  simp only [IsWeaklyClosed] at hs
...
lemma norm_sq_eq_inner (a b : H) : ‖a + b‖^2 = ‖a‖^2 + ‖b‖^2 + 2 * ⟪a,b⟫ := by
  calc
    ...
    _ = ⟪a, a⟫ + ⟪b, b⟫ + 2 * ⟪a,b⟫ := by
      rw [real_inner_add_add_self, add_assoc]
      nth_rw 2 [add_comm]
      rw [← add_assoc]
    _ = ‖a‖^2 + ‖b‖^2 + 2 * ⟪a,b⟫ := by
...
theorem browder_demiclosed_principle [CompleteSpace H]
      ...
      - 2 * ⟪x n - x_lim, x_lim - T x_lim - u⟫ := by
        ...
        calc
          ...
          _ = ‖x n - T x_lim - u‖ ^ 2 - ‖x n - x_lim‖ ^ 2
              - 2 * ⟪x n - x_lim, x_lim - T x_lim - u⟫ := by
            simp [mul_add, ← add_assoc]; ring_nf; simp only [add_comm, add_sub]
            congr 3
            · simp only [norm_nonneg, ne_eq, OfNat.ofNat_ne_zero, not_false_eq_true, pow_left_inj₀]
          ...
          _ ≤ _ := by
            have : ‖T (x n) - T x_lim‖ ^ 2 ≤ ‖x n - x_lim‖ ^ 2 := by
              apply sq_le_sq.2; simp only [abs_norm]
              rw [NonexpansiveOn, LipschitzOnWith] at hT_nonexp
              have := hT_nonexp (hx_in_D n) hx_lim_in_D
              simp only [edist_dist, ENNReal.coe_one, one_mul, dist_nonneg,
                ENNReal.ofReal_le_ofReal_iff] at this;
  ...
  have h4 : WeakConverge (fun n => x n - T (x n)) u := by
    ...
    · have h2' : Tendsto (fun n => (x n - T (x n)) - u) atTop (𝓝 0) := by
      ...
      have h_inner : Tendsto (fun n => ⟪(x n - T (x n)) - u, y⟫) atTop (𝓝 0) := by
        ...
        intro n hn
        specialize hN n hn
        simp only [dist_eq_norm, sub_zero, Real.norm_eq_abs] at hN ⊢
        by_cases hy : y = 0
        · simp [hy]; linarith
        · calc
            |⟪(x n - T (x n)) - u, y⟫|
  ...
  have h4 : WeakConverge (fun n => T (x n) - x n) (- u) := by
    ...
    · ext n; simp only [sub_neg_eq_add]
      rw [← inner_neg_left]; simp [inner_sub_left, inner_add_left]; ring_nf
    simp
